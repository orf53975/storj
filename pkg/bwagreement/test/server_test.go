...
func TestSameSerialNumberBandwidthAgreements(t *testing.T) {
	satellitedbtest.Run(t, func(t *testing.T, db satellite.DB) {
		...
		server := bwagreement.NewServer(db.BandwidthAgreement(), db.UplinkDB(), zap.NewNop(), satellitePubKey)

		pbaFile1, err := GeneratePayerBandwidthAllocation(ctx, db.UplinkDB(), pb.PayerBandwidthAllocation_GET, satellitePrivKey, uplinkPrivKey, false)
		...
		pbaFile2, err := GeneratePayerBandwidthAllocation(ctx, db.UplinkDB(), pb.PayerBandwidthAllocation_GET, satellitePrivKey, uplinkPrivKey, false)
		assert.NoError(t, err)
...
func TestManipulatedBandwidthAgreements(t *testing.T) {
	satellitedbtest.Run(t, func(t *testing.T, db satellite.DB) {
		...
		pba, err := GeneratePayerBandwidthAllocation(ctx, db.UplinkDB(), pb.PayerBandwidthAllocation_GET, satellitePrivKey, uplinkPrivKey, false)
		assert.NoError(t, err)
		...
		expPBA, err := GeneratePayerBandwidthAllocation(ctx, db.UplinkDB(), pb.PayerBandwidthAllocation_GET, satellitePrivKey, uplinkPrivKey, true)
		assert.NoError(t, err)
